...
const INSTANCE_GROUPS = new Map(
    [
        [
            String(process.env.EVOLUTION_INSTANCE_1 || '').trim(),
            parseGroupList(process.env.EVOLUTION_INSTANCE_1_GROUPS),
        ],
        [
            String(process.env.EVOLUTION_INSTANCE_2 || '').trim(),
            parseGroupList(process.env.EVOLUTION_INSTANCE_2_GROUPS),
        ],
...
function getWebhookInstance(req) {
    ...
    const pathMatch = String(req.originalUrl || '')
        .split('?')[0]
        .match(/^\/webhook\/([^/]+)$/);

    const pathInstance = pathMatch?.[1]
        ? decodeURIComponent(pathMatch[1])
        : '';

    return String(bodyInstance || pathInstance || '').trim();
}
...
app.post(['/webhook', '/webhook/*'], async (req, res) => {
    ...
    const instanceName = getWebhookInstance(req);
    ...
    try {
        ...
        if (!allowedGroupsForInstance) {
            console.log('SKIP: unknown Evolution instance', {
                instanceName,
                configuredInstances: [...INSTANCE_GROUPS.keys()],
            });
            return;
        }
        ...
        console.log('INSTANCE/GROUP ACCEPTED', {
            instanceName,
            remoteJid: env.remoteJid,
        });
